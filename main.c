...
static ko_longopt_t long_options[] = {
	...
	{ (char*)"bin-size",			ko_required_argument, 	401 },
	{ (char*)"overlap",				ko_required_argument, 	408 },
#ifndef NGRPCRH
...
int main(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		...
		else if (c == 401) opt.diag_bin_size = atoi(o.arg); // --bin-size
		else if (c == 408) { // --overlap
			int overlap_mode = atoi(o.arg);
			if (overlap_mode == 0 || overlap_mode == 1) opt.diag_overlap = overlap_mode;
			else {
				fprintf(stderr, "[ERROR] --overlap must be 0 or 1.\n");
				return 1;
			}
		}
#ifndef NGRPCRH
	...
	if (opt.diag_bin_size < -1) {
		fprintf(stderr, "[ERROR] --bin-size must be >= 0.\n");
		return 1;
	}
	if (opt.diag_bin_size >= 0) {
		opt.flag |= RI_M_DIAG_MAPPING;
		if (ipt.w != 0) {
			fprintf(stderr, "[ERROR] fast offset mode currently requires -w 0.\n");
			return 1;
		}
		if (opt.flag & RI_M_SEQUENCEUNTIL) {
			fprintf(stderr, "[ERROR] fast offset mode does not support --sequence-until.\n");
			return 1;
		}
#ifndef NGRPCRH
		if (live_mode) {
			fprintf(stderr, "[ERROR] fast offset mode does not support --live.\n");
			return 1;
		}
#endif
	}
	...
	if (argc == o.ind || fp_help == stdout) {
		...
		fprintf(fp_help, "    --bin-size INT         enable fast offset mode; 0 => exact d_pos, >0 => floor(d_pos/INT) [-1=disabled]\n");
		fprintf(fp_help, "    --overlap INT          in fast offset mode: 0 => raw seed-hit counts, 1 => overlap-aware query coverage [%d]\n", opt.diag_overlap);
