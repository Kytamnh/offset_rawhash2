...
#define RI_M_DIAG_MAPPING	0x8000
...
typedef struct ri_mapopt_s{
	...
	//Fast offset-mapping mode
	int32_t diag_bin_size;   // -1 disables offset mode; 0 => exact d_pos keys; >0 => floor(d_pos/bin_size)
	int32_t diag_overlap;    // 0 => raw seed-hit counts, 1 => overlap-aware query coverage
} ri_mapopt_t;
