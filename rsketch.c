...
static inline uint64_t ri_overlap_pack_q_bounds(uint32_t q_start, uint32_t q_end)
{
	return ((uint64_t)q_end << 32) | q_start;
}

void ri_sketch_query_overlap(void *km,
							 const float* s_values,
							 uint32_t len,
							 float diff,
							 int e,
							 ...
							 mm128_v *p)
{
	const uint64_t hash_mask = (1ULL << 32) - 1;
	const uint64_t mask_quant_bit = (1ULL << quant_bit) - 1;
	const uint32_t n_buckets = 1UL << quant_bit;
	const uint32_t total_bits = (uint32_t)e * quant_bit;
	const uint64_t mask_events = total_bits >= 64 ? UINT64_MAX : ((1ULL << total_bits) - 1);
	uint32_t pos_buf[256];
	int sig_buf_full = 0;
	uint32_t f_pos, pos_buf_pos = 0, l_sigpos = 0;
	uint32_t f_tmpQuantSignal = 0;
	uint64_t quantVal = 0;

	assert(len > 0 && e > 0 && e < 256 && total_bits <= 64);
	memset(pos_buf, 0, (size_t)e * sizeof(uint32_t));
	...
	for (f_pos = 0; f_pos < len; ++f_pos) {
		...
		pos_buf[pos_buf_pos] = f_pos;
		if (++pos_buf_pos == (uint32_t)e) { sig_buf_full = 1; pos_buf_pos = 0; }
		if (!sig_buf_full) continue;

		q_start = pos_buf[pos_buf_pos];
		info.x = hash64(quantVal, hash_mask);
		info.y = ri_overlap_pack_q_bounds(q_start, f_pos);
